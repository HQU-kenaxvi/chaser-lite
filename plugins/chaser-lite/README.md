...
| `/chaser-lite:integrer` | Étend le compresseur à **33 agents de code** : Cursor & Gemini CLI (hooks officiels, deny-with-suggestion) + Copilot, Windsurf, Cline, Roo, Codex, Aider, Zed, Warp, Amazon Q, Continue, Kilo Code… (fichiers de règles + standard ouvert AGENTS.md). `--tous` pour tout poser d'un coup. Par projet, **réversible et sans trace** via `/chaser-lite:integrer retirer`. |
